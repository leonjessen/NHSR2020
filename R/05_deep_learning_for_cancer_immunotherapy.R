...
acc_test <- round(perf_test["accuracy"], 3)*100
perf_train <- model %>%
  evaluate(X_train, y_train)
acc_train <- round(perf_train["accuracy"], 3)*100
results <- bind_rows(
...
results %>%
  ...
  labs(
    title = str_c("Performance of Deep Feed Forward Neural Network (",
                  "Total number of model parameters = ", count_params(model), ")."),
    subtitle = sub_title <- str_c("Test Accuracy = ", acc_test, "%, n = ", nrow(X_test), ". ",
                                  "Training Accuracy = ", acc_train, "%, n = ", nrow(X_train), "."),
    x = "Predicted (Class assigned by Keras/TensorFlow deep FFN)",
    y = "Measured (Real class, as predicted by netMHCpan-4.0)"
  ) +
  theme_bw() +
